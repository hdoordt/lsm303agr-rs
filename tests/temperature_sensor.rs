...
async fn can_read_temperature_has_new_data() {
    ...
    assert!(sensor.temperature_status().await.unwrap().new_data());
    destroy_i2c(sensor);
}
...
async fn can_read_temperature_has_data_overrun() {
    let mut sensor = new_i2c(&[I2cTrans::write_read(
        ACCEL_ADDR,
        vec![Register::STATUS_REG_AUX_A],
        vec![BF::TOR],
    ...
    assert!(sensor.temperature_status().await.unwrap().overrun());
    destroy_i2c(sensor);
}
...
async fn can_read_temperature_has_no_new_data() {
    let mut sensor = new_i2c(&[I2cTrans::write_read(
        ACCEL_ADDR,
        vec![Register::STATUS_REG_AUX_A],
        vec![0x00],
...
async fn can_read_temperature_i2c() {
    ...
    sensor
        ...
        .await
        .unwrap();
    let data = sensor.temperature().await.unwrap();
...
async fn can_read_temperature_spi() {
